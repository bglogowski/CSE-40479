...
public class FindPrimes_test extends TestCase {
    ...
    public void testExhaustiveOutputVerification() {
        ...
        try {
            ...
            /**
             * Loop 10000, checking every number along the way for primeness... If a given
             * number is prime, ensure that it appears in the output of the code under test
             */
            for (int ii = 0; ii < 10000; ii++) {
                if (isPrime(ii)) {
                    assertEquals("The prime numbers table data is incorrect and/or incorrectly formatted.", ii,
                            sc.nextInt());
    ...
    // This should be overkill (the program's output is exhaustively verified in
    // another test), but will confirm that the isPrime method was declared as
    // specified and will help identify candidates for partial credit
    ...
    public static boolean isPrime(int num) {
        ...
        /**
         * I've commented out the less efficient approach... for(int i=2; i <= num/2;
         * i++) {
         * 
         * This exercise shows that you can solve the same problem with different
         * algorithms... the key takeaway is that, just because one approach works
         * doesn't mean it is the best.
         *
         * In this case, all of the numbers > the square root must be multiplied by a
         * number < the square root to equal the target number... so why not just check
         * the numbers <= the square root?
         */
     ...
     * @param str
     *            the string to identify within the candidate lines (Scanner's
     *            buffer). It is converted to lower case in support of a
     *            case-insensitive search.
     * @return the next line from the scanner containing the specified parameter or
    ...
    /*
     * method name retrieval code courtesy of:
    ...
    private void assertThrowableTestFailure(Throwable thrown) {
        ...
        fail(thrown.getClass().getName() + " encountered! Unable to successfully execute test: "
